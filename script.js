...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        ...
        const nameInput = contactForm.querySelector('input[name="name"], input[placeholder*="name" i], input[placeholder*="اسم" i]');
        const messageInput = contactForm.querySelector('textarea[name="message"], textarea[placeholder*="message" i], textarea[placeholder*="رسالة" i]');
        ...
        if (emailInput) {
            emailInput.addEventListener('blur', function() {
                validateField(emailInput, validateEmail, 'يرجى إدخال بريد إلكتروني صحيح');
            });
        ...
        if (nameInput) {
            nameInput.addEventListener('blur', function() {
                validateField(nameInput, null, '');
            });
            
            nameInput.addEventListener('input', function() {
                if (nameInput.parentElement.querySelector('.form-error')) {
                    validateField(nameInput, null, '');
                }
            });
        }
        ...
        if (messageInput) {
            messageInput.addEventListener('blur', function() {
                validateField(messageInput, null, '');
            });
            
            messageInput.addEventListener('input', function() {
                if (messageInput.parentElement.querySelector('.form-error')) {
                    validateField(messageInput, null, '');
                }
            });
        }
        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            const nameValid = validateField(nameInput, null, '');
            const emailValid = validateField(emailInput, validateEmail, 'يرجى إدخال بريد إلكتروني صحيح');
            const messageValid = validateField(messageInput, null, '');
            ...
            if (!isValid) {
                ...
                if (firstError) {
                    const inputWithError = firstError.parentElement.querySelector('input, textarea');
                    if (inputWithError) {
    ...
    if (animatedElements.length > 0) {
        ...
        if (!('IntersectionObserver' in window)) {
            ...
            let scrollTimeoutFallback;
            window.addEventListener('scroll', function() {
                if (scrollTimeoutFallback) {
                    clearTimeout(scrollTimeoutFallback);
                }
                scrollTimeoutFallback = setTimeout(checkVisibility, 50);
            }, { passive: true });
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '06cfb43a-65fe-4f58-8d39-4d9566303fca',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '06cfb43a-65fe-4f58-8d39-4d9566303fca';
...
/* ZAPPY_CUSTOM_JS_START:ce1cff17de9b */
(function () {
  function __zappyCustomInit() {
    try {
document.addEventListener('DOMContentLoaded', function() {
  const faqList = document.querySelector('.faq-faq-list-section__list');
  if (!faqList) return;
...
/* ZAPPY_CUSTOM_JS_END:ce1cff17de9b */

/* ZAPPY_CUSTOM_JS_START:bde40a15a8e8 */
(function () {
  function __zappyCustomInit() {
    try {
(function() {
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '06cfb43a-65fe-4f58-8d39-4d9566303fca';
